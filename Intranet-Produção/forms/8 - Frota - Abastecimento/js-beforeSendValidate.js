...
function validaCampos(numState, nextState){
	...
	if (isEmpty("geraisCodMotorista", form)) {
    	...
    	hasErros = true;
    	$('#geraisCodMotorista').closest(".form-group").addClass("has-error");
    }
	if (isEmpty("geraisCPFMotorista", form)) {
    	message += getMessage("CPF Motorista", 1, form);
    	hasErros = true;
    	$('#geraisCPFMotorista').closest(".form-group").addClass("has-error");
    }
    ...
    if (isEmpty("geraisVeiculo", form)) {
		...
		hasErros = true;
		$('#geraisVeiculo').closest(".form-group").addClass("has-error");
	}
    ...
    if (isEmpty("abastTpCombustivel", form)) {
    	message += getMessage("Tipo de Combustível", 1, form);
    	hasErros = true;
    	$('#abastTpCombustivel').closest(".form-group").addClass("has-error");
    }

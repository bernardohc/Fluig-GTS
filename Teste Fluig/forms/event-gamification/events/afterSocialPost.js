function afterSocialPost(companyId, post){
	try{
		...
		log.info("New Gamification: afterSocialPost");
		...
		var jsonContent = {};
		jsonContent.collaboratorAlias = post.getUser();
		jsonContent.collaboratorTeam = 'comunicacao';
		jsonContent.eventCode = 'new-post';
		...
		log.info(data);
		log.info("END Gamification: afterSocialPost");

	}catch (e) {
		log.error("## ERROR Gamification: afterSocialPost");
		log.error(e);
	}
}
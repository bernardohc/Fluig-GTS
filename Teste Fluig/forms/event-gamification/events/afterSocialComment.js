function afterSocialComment(companyId, comment){
	try{
		...
		log.info("New Gamification: afterSocialComment");
		...
		var jsonContent = {};
		jsonContent.collaboratorAlias = comment.getUser();
		jsonContent.collaboratorTeam = 'comunicacao';
		jsonContent.eventCode = 'new-comment';
		...
		log.info(data);
		log.info("END Gamification: afterSocialComment");

	}catch (e) {
		log.error("## ERROR Gamification: afterSocialComment");
		log.error(e);
	}
}
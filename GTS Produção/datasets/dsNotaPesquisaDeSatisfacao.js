function createDataset(fields, constraints, sortFields) {
	...
	if( numSerie != ""  ){
		var SQL =	" SELECT pesqNotaAtendimento ,pesquisa.pesqNumSerie " +
					...
					" 	AND pesquisa.version = (SELECT max(version) FROM ML001097 entrega_SUB WHERE entrega_SUB.documentid = pesquisa.documentid )" +
					" INNER JOIN (" +
					...
					" 	(SELECT max(version) FROM ML001097 MLTB WHERE MLTB.documentid = pesquisa.documentid)" +

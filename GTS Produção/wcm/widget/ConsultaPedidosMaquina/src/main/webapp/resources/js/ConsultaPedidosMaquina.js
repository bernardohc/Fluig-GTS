...
//		$("#dadosPedido").hide();
	...
	loadData: function(){
		...
		getDataset("dsPedMaqConsultaPedidos", constraintsConsultaPedidos, function(data){
			...
			if (data != null && data.content != null && data.content.values.length > 0) {		
	    		...
	    		for( var index in records) {
		            ...
		            if(record.CODRET.trim() == '1'){
			            ...
			            var acoes = `<a data-visualizar_pedido id="${index}" alt="Vizualizar Pedido" title="Vizualizar Pedido" ><i class="fluigicon fluigicon-search icon-md" style="cursor: pointer;"></i></a>`;
			...
			that.myTable = FLUIGC.datatable('#tabelaPedidos_' + instanceId , {
				...
				mobileMainColumns: [0,1,3,4,6,8,9,10,17,25],
				header: [{
					'title': '',
					'size': 'th-icon-acoes'
				},{
			...
			//Documentação para possibilitar ordenar pela coluna
			//https://www.todoespacoonline.com/w/2014/09/tablesorter-jquery/
			//https://mottie.github.io/tablesorter/docs/#Demo
			$('#tabelaPedidos_' + instanceId + ' .table').tablesorter({
				// Envia os cabeçalhos 
				headers: { 
		        	0: { 
		        		//Coluna de Busca (icone de detalhes)
		                sorter: false 
		            },
		            1: { 
		            	//Status
			...
			$('#tabelaPedidos_' + instanceId + ' .table').addClass('tablesorter');
	...
	visualizarPedido: function(el, ev){
    	...
    	var selected = this.myTable.getRow(el.id);

...
function zooms(ev){
		...
		if ( ev == 'selecionaRevendeda' ) {
			//function(dataset, fields, resultfields, title, filters, type, likefield, likevalue, searchby)
			modalzoom.open("dsConsultaRevendas", "A1_COD,Cod. ,A1_LOJA,Loja,A1_NOME,Filial,A1_CGC,CPF/CNPJ","A1_COD,A1_LOJA,A1_NOME,A1_CGC", "Revenda", null , 'selecionaRevendeda' , "" , "", "" );
		}
...
function setSelectedZoomItem(selectedItem) {
		...
		if (tipo == "selecionaRevendeda" ) {
			$('#codRevenda').val(selectedItem.A1_COD);
			$('#lojaRevenda').val(selectedItem.A1_LOJA);
			$('#nomeRevenda').val(selectedItem.A1_MUN.trim() + ' - ' +selectedItem.A1_NOME.trim());

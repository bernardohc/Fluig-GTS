...
public class ConnectRest {
    ...
    private void config(String domain) {
        try {
            createConnect(domain, "integrador.de.sistemas", "integr@d0rFlu1g");
        } catch (Exception e) {
            e.printStackTrace();
        }
...
//                System.out.println("TOKEN SECRET: " + oAuthClient.getTokenSecret());
    ...
    @POST
    @Path("/dsConsultaSACByProtocolo")
    @Produces(MediaType.APPLICATION_JSON)
    @Consumes(MediaType.APPLICATION_JSON)
    public Response postDSConsultaSACByProtocolo(String dataParam) throws IOException, OAuthCommunicationException, OAuthExpectationFailedException, OAuthMessageSignerException {
    	...
    	try {
            ...
            //Aqui força para buscar do dataset dsConsultaSACByProtocolo
            String dataParams = (String) jsonObj.get("parms");  
            JSONObject jsonObjDataParam = new JSONObject(dataParams);
            jsonObjDataParam.put("name", "dsConsultaSACByProtocolo");
...
//            System.out.println(String.format("RESPONSE: %d - %s: data: %s", code, conn.getResponseMessage(), retCreateUser));
    ...
    @POST
    @Path("/dsConsultaSACAtendimentoExterno")
    @Produces(MediaType.APPLICATION_JSON)
    @Consumes(MediaType.APPLICATION_JSON)
    public Response postDSConsultaSACAtendimentoExterno(String dataParam) throws IOException, OAuthCommunicationException, OAuthExpectationFailedException, OAuthMessageSignerException {
    	...
    	try {
            ...
            //Aqui força para buscar do dataset dsConsultaSACAtendimentoExterno
            String dataParams = (String) jsonObj.get("parms");  
            JSONObject jsonObjDataParam = new JSONObject(dataParams);
            jsonObjDataParam.put("name", "dsConsultaSACAtendimentoExterno");

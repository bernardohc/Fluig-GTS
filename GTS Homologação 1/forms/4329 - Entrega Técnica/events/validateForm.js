    		...
    		if (getValue("WKCompletTask") == "true" ){
	    		...
	    		if (form.getValue("tipoSolicitante") != 'Administrativo GTS') {
		    		if (isEmpty("revRazaoSocialRevenda", form)) {
		    			message += getMessage("Razão Social da Loja", 1, form);
		    			hasErros = true;
		    		}
		    		if (isEmpty("revNomeFantasiaRevenda", form)) {
		    			message += getMessage("Nome Fantasia da Loja", 1, form);
		    			hasErros = true;
		    		}
		    		if (isEmpty("revCpfCnpj", form)) {
		    			message += getMessage("CPF/CNPJ da Loja", 1, form);
		    			hasErros = true;
		    		}
		    		if (isEmpty("revCodigo", form)) {
		    			message += getMessage("Código da Loja", 1, form);
		    			hasErros = true;
		    		}
		    		if (isEmpty("revLoja", form)) {
		    			message += getMessage("Loja da Loja", 1, form);
		    			hasErros = true;
		    		}
		    		if (isEmpty("revCidade", form)) {
		    			message += getMessage("Cidade da Loja", 1, form);
		    			hasErros = true;
		    		}
		    		if (isEmpty("revEstadoHidden", form)) {
		    			message += getMessage("Estado da Loja", 1, form);
		    			hasErros = true;
		    		}

...
var funcoes = (function() {
	...
	return {
		...
		consultaProdutoQtdUSAAssync : function(codProduto, Qtde, limparCampos, desc, flDesc, liberaAddTabela){
		...
		consultaProdutoQtdAssync : function(codProduto, Qtde, limparCampos, desc, flDesc, liberaAddTabela){
...
//			 PercentDesconto = validafunctions.getFloatValue("PercentDesconto");
		...
		carregaDadosUsuario : function(){
			
			$.ajax({
	    		type: "GET",
	    		dataType: "json",
	    		async: false,
	    		url: "/api/public/ecm/dataset/search?datasetId=dsConsultaClienteWS",
	    		data: "",
	    	    success: function (data, status, xhr) {
//	    	    	console.log(data)
	    	    	if (data != null && data.content != null && data.content.length > 0 ) {
	    	    		
	    	    		/*if(data.content[0].ERROR != null){
	    	    			var mensagem = data.content[0].ERROR;
			            	FLUIGC.toast({ title: '', message: mensagem, type: 'danger' });
							funcoes.limpaCampos();
							$('#divOrcamentoPedido').hide();
							$('#divOrcamentoCliente').hide();
							$('#divOrcamentoProduto').hide();
							$('#divOrcamentoTabItens').hide();
	    	    		}else{*/
    	    			...
    	    			if( records[0].CODRET == "1"){
	    		            	...
	    		            	var NOME = record.A1_NOME;
		    					var CNPJ = record.A1_CGC;
		    					var IE = record.A1_CGC;
		    					var Municipio = record.A1_MUN;
		    					var Estado = record.A1_EST;
		    					var Endereco = record.A1_END;
		    					var Bairro = record.A1_BAIRRO;
		    					var CEP = record.A1_CEP;
		    					var desconto = record.A1_DESC;
		    					var riscoRevenda = record.A1_RISCO;
			    					
						    	if(NOME != ""){
						    		$("#endNomeCliente").val(NOME);
						    	}
						    	if(CNPJ != ""){
						    		$("#endCNPJ").val(CNPJ);
						    	}
						    	if(Municipio != ""){
						    		$("#endCidade").val(Municipio);
						    	}
						    	if(Estado != ""){
						    		$("#endUF").val(Estado);
						    	}
						    	...
						    	if(desconto != ""){
						    		$("#descontoUsuario").val(desconto);
						    	}
						    	if(riscoRevenda != ""){
						    		$("#riscoRevenda").val(riscoRevenda);
						    	}
	    		            }else if (records[0].CODRET == "2"){
			    	    		
	    		            	var msg_erro_consulta_revenda = $('#msg_erro_consulta_revenda').text();
	    		            	FLUIGC.toast({ title: '', message: msg_erro_consulta_revenda, type: 'danger' });
	    		            	console.log(records[0].MSG);
								funcoes.limpaCampos();
								$('form').hide();
								$('#divDefineExportacaoPedido').hide();
								$('#divOrcamentoPedido').hide();
								$('#divOrcamentoCliente').hide();
								$('#divOrcamentoProduto').hide();
								$('#divOrcamentoTabItens').hide();
    		        	...
    		        	FLUIGC.toast({ title: '', message: 'O cliente não está cadastrado corretamente na base de dados!', type: 'warning' });
						funcoes.limpaCampos();
						$('#divOrcamentoPedido').hide();
						$('#divOrcamentoCliente').hide();
						$('#divOrcamentoProduto').hide();
    					...
    					if(A1_TIPO.toUpperCase() == 'GERENTE'){
    						...
    						if(defineOrcamentoPedido == 'orcamento'){
    							...
    							if(CURRENT_STATE == INICIO_0){
    								...
    								$('#divBtnImprimir').addClass('col-md-offset-8');
    							}else{
    								$('#divTotalVenda').addClass('col-md-offset-10');
    							}
    							...
    							if(CURRENT_STATE == INICIO_0){
    								...
    								$('#divBtnImprimir').removeClass('col-md-offset-8');
    							}else{
    								$('#divTotalVenda').removeClass('col-md-offset-10');
    							}
		...
		setup : function() {	
			...
			$(document).on("change", "#chkRedespacho", function() {
				if( $('#chkRedespacho').prop("checked") ){
					$('#nomeRedespacho').prop('readonly', false);
				}else{
					$('#nomeRedespacho').val('');
					$('#nomeRedespacho').prop('readonly', true);
				}
			});
			...
			$(document).on("change", "#orcProdCod", function() {
				if($('#orcProdCod').val() != ''){
					...
					if(A1_PAIS == 'USA'){
						...
						funcoes.consultaProdutoQtdUSAAssync( $("#orcProdCod" ).val(), $("#orcProdQtd" ).val(), false, desc, flDesc, false )
						...
						funcoes.consultaProdutoQtdAssync( $("#orcProdCod" ).val(), $("#orcProdQtd" ).val(), false, desc, flDesc, false )
			...
			$(document).on("change", "#orcProdQtd", function() {
				...
				if($("#orcProdCod" ).val() == ''){
					...
					if(A1_PAIS == 'USA'){
						...
						funcoes.consultaProdutoQtdUSAAssync( $("#orcProdCod" ).val(), $("#orcProdQtd" ).val(), false, desc, flDesc, true )
						...
						funcoes.consultaProdutoQtdAssync( $("#orcProdCod" ).val(),  $("#orcProdQtd" ).val(), false, desc, flDesc, true );
					}
				}
			...
			$(document).on("click","#btnExcelPartes", function() {
				...
				const cab = [  'Qtd', 'Cod. Produto', 'Desc. Produto', 
				        'Preço Custo Real', 'Preço Custo Dolar', 
				        'Total Custo Real', 'Total Custo Dolar'  ];
				data.push(cab);
				...
				$("input[name*=orcCodProdutoItem___]").each(function(index){
					...
					var orcQtdItem = $('#orcQtdItem___'+ index).val();
					var orcCodProdutoItem = $('#orcCodProdutoItem___'+ index).val();
					var orcDescProdutoUSAItem = $('#orcDescProdutoItem___'+ index).val();
					var orcPrecoCustoItem = $('#orcPrecoCustoItem___'+ index).val();
					var orcPrecoCustoDolarItem = $('#orcPrecoCustoDolarItem___'+ index).val();
					var orcTotalCustoComImpItem = $('#orcTotalCustoComImpItem___'+ index).val();
					var orcTotalCustoDolarItem = $('#orcTotalCustoDolarItem___'+ index).val();
				...
				hiddenElement.download = 'Pedido Id Fluig - '+ WKNumProces + '.csv';
				hiddenElement.click();
					*/
			});
...
function loadForm(){
	...
	if($('#origemSolicitacao').val()=='catalogo'){
		...
		$('#linkCatalogo').attr('href', 'https://teste.siscatec.com/BudgetOrders/Details/' + $('#numCatalogo').val());
	}
	...
	{	
		...
		if($('#origemSolicitacao').val() == 'catalogo' && $('#itensProcessados').val() != 'true' ){
			...
			//chama funcao carregaDadosUsuario para alimentar os campos do usuário de abertura.
			funcoes.carregaDadosUsuario();
...
//		reloadZoomFilterValues('CPProgramacaoRetirada_3', "seqCondPgto," + selectedItem["seqCondPgto"]);

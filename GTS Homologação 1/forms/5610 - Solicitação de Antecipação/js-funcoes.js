		...
		setup : function() {
			...
			$(document).on("click", "#addMaquina",function() {
				...
				$("input[name*=solPedido___]").each(function(index) {
					...
					var solPrazo = $("#solPrazo___"+ index).val();
					...
					if (!addNovoItem) {
						FLUIGC.toast({title : '',message : "É preciso preencher a antecipação anterior.",type : 'warning'});
					}else{
...
function loadForm() {
	...
	} else if (CURRENT_STATE == ANALISA_ANTECIPACAO) {

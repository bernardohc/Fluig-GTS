...
//		if ( constraints[i].getFieldName().toString() == 'nomeGrupoTerritorial' ) {
//			constGrupoTerritorial = constraints[i].initialValue;
//		}
//	}
	...
	//Se for Gestor Comercial, ainda busca os usuários que são exportação
	...
	if(dsTemValor(dsColleagueRoleGestorComercial)){
		var colleagueIdGestorComercial = dsColleagueRoleGestorComercial.getValue(0, "gesComWKUser");
		var nomeGestorComercial = dsColleagueRoleGestorComercial.getValue(0, "gesComNome");
		...
		if(dsTemValor(dsGruposExp)){
			for ( var j = 0; j < dsGruposExp.rowsCount; j++) {
				...
				var fieldsColRepExp = ["colleagueName"];

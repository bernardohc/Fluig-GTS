...
function createDataset(fields, constraints, sortFields) {
	...
	newDataset.addColumn("A1_CODLOJA");
	newDataset.addColumn("A1_NOME");
	newDataset.addColumn("A1_CGC");
	newDataset.addColumn("A1_PESSOA");
	...
	for (var i in constraints){
		...
		if ( constraints[i].getFieldName().toString() == 'CODCIDADE' ) {
			CODCIDADE = constraints[i].initialValue;
		}
	}

	var SQL = " select A1_COD, A1_LOJA, CONVERT(VARCHAR, A1_COD + '-' + A1_LOJA) A1_CODLOJA,  A1_NOME, A1_MUN , A1_EST , A1_TABELA ,  A1_END , A1_BAIRRO, " +
	 		...
	 		" and A1_TIPO =  'R' " +
	 		" and A1_EST =  '" + UF + "' " +
	 		" and A1_COD_MUN =  '" + CODCIDADE + "' " +
	 		" order by A1_NOME ";

	 log.info(" sqlConsultaTodasRevendas :"+ SQL);
	 ...
	 while(rsWD.next()){
		 newDataset.addRow(
				 new Array(
					...
					rsWD.getString("A1_CODLOJA"),
					rsWD.getString("A1_NOME"),
					rsWD.getString("A1_CGC"),
					rsWD.getString("A1_PESSOA"),
